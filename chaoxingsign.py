...
class CxSign():  # 尝试写一下class.....结果发现写的乱七八糟......
    def __init__(self,conf_user):  # 转换一下配置文件
        self.username = conf_user['username']  # 根据列表顺序来读取
        self.passwd = conf_user['passwd']
        self.SCKEY = conf_user['SCKEY']  # 那么所有的用户都用这一个参数
    ...
    def login(self):  # 获取cookie,传入用户的列表位置,所有的参数都能对上...(应该能吧....)
